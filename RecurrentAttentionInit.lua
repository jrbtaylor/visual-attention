...
------------------------------------------------------------------------
local RecurrentAttentionInit, parent = torch.class("nn.RecurrentAttentionInit", "nn.AbstractSequencer")

function RecurrentAttentionInit:__init(init, rnn, action, nStep)--, hiddenSize)
   parent.__init(self)
...
function RecurrentAttentionInit:updateOutput(input)
   self.rnn:forget()
   self.action:forget()
...
function RecurrentAttentionInit:updateGradInput(input, gradOutput)
...
function RecurrentAttentionInit:accGradParameters(input, gradOutput, scale)
   assert(self.rnn.step - 1 == self.nStep, "inconsistent rnn steps")
   assert(torch.type(gradOutput) == 'table', "expecting gradOutput table")
   assert(#gradOutput == self.nStep, "gradOutput should have nStep elements")
...
function RecurrentAttentionInit:accUpdateGradParameters(input, gradOutput, lr)
   assert(self.rnn.step - 1 == self.nStep, "inconsistent rnn steps")
   assert(torch.type(gradOutput) == 'table', "expecting gradOutput table")
   assert(#gradOutput == self.nStep, "gradOutput should have nStep elements")
...
function RecurrentAttentionInit:type(type)
   self._input = nil
   self._actions = nil
   self._crop = nil
   self._pad = nil
...
function RecurrentAttentionInit:__tostring__()
   local tab = '  '
   local line = '\n'
   local ext = '  |    '
   local extlast = '       '
