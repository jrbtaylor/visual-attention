...
------------------------------------------------------------------------
local RecurrentAttentionInitAug, parent = torch.class("nn.RecurrentAttentionInitAug", "nn.AbstractSequencer")

function RecurrentAttentionInitAug:__init(init, rnn, action, augment, nStep)--, hiddenSize)
   parent.__init(self)
   ...
   self.modules = {self.augment, self.rnn, self.action}
...
function RecurrentAttentionInitAug:updateOutput(input)  
   self.rnn:forget()
   self.action:forget()
   local nDim = input:dim()
   
   input = self.augment:updateOutput(input)
   self.input = input
...
function RecurrentAttentionInitAug:updateGradInput(input, gradOutput)
  
  input = self.input--self.augment:updateOutput(input)
...
function RecurrentAttentionInitAug:accGradParameters(input, gradOutput, scale)
...
function RecurrentAttentionInitAug:accUpdateGradParameters(input, gradOutput, lr)
...
function RecurrentAttentionInitAug:type(type)
   self._input = nil
   self._actions = nil
   self._crop = nil
   self._pad = nil
...
function RecurrentAttentionInitAug:__tostring__()
   local tab = '  '
   local line = '\n'
   local ext = '  |    '
   local extlast = '       '
   ...
   str = str .. ' {'
   str = str .. line .. tab .. 'augment : ' .. tostring(self.augment):gsub(line, line .. tab .. ext)
